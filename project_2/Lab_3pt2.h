...
DELAY3:
    ...
    // POST LAB
// 1.
    // 8 bits: 2^8 - 1 = 255 -- maximum time: 255 * 0.1 ms = 25.5 ms
    // 16 bits: 2^16 - 1 = 65535 -- maximum time: 65535 * 0.1 ms = 6553.5 ms
    // 24 bits: 2^24 - 1 = 16777215 -- maximum time: 16777215 * 0.1 ms = 1677721.5 ms
    // 32 bits: 2^32 - 1 = 4294967295 -- maximum time: 4294967295 * 0.1 ms = 429496729.5 ms


// 2.
    // Human reaction time ranges from 200 ms to a few seconds. Therefore, using the 16 bit register allows for an accurate reaction time for this task. 
    // This is because 6553.5 ms is approximately 6.5 seconds, which lies in the range for typical human response time.

// 3.
    // The code generates a delay between 2 - 10 seconds. It counts up by 0.1ms, so the the delay value ranges from 20000 - 99999.
    // +/- 5%: 
        // 2 seconds +/- 5% = 1.9 to 2.1 seconds
        // 10 seconds +/- 5% = 9.5 to 10.5 seconds